...
if(isset($_SESSION['ut']) &&(strlen($_SESSION['ut'])>0) ){
    if(isset($_SESSION['controllorichiesta']) && (strcmp($_SESSION['controllorichiesta'],"ok")==0) ){
        ...
        if(isset($_POST['fasciaOra'])&& isset($_POST['descrizioneOra']) && isset($_POST['idTurno'])&& (strlen($_POST['fasciaOra'])>0) && (strlen($_POST['descrizioneOra'])>0)&& strlen($_POST['idTurno'])>0)
        {
            include_once('RichiestaCambioOrario.php');
            include_once('GestoreRichiestaCambioOrario.php');


            $cambioOrario = new RichiestaCambioOrario($_POST['fasciaOra'],$_POST['descrizioneOra'],$_SESSION['ut'],$_POST['idTurno']);
